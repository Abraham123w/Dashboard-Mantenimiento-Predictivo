...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // NUEVO: Carga automática del archivo por defecto al iniciar la página
    loadCSV('plan_mantenimiento_futuro_enriquecido.csv');
});


// MODIFICADO: Función centralizada para cargar y procesar cualquier CSV
function loadCSV(source) {
    console.log(`Cargando y procesando CSV desde:`, source);
    const isUrl = typeof source === 'string'; // Detecta si la fuente es una URL o un archivo

    Papa.parse(source, {
        header: true,
        delimiter: ";",
        skipEmptyLines: true,
        download: isUrl, // NUEVO: Esencial para que PapaParse descargue el archivo si es una URL
        complete: function(results) {
            ...
            // Damos un mensaje más general que funciona para ambas cargas
            console.log(`Carga exitosa: ${results.data.length} registros procesados.`);
        },
...
// MODIFICADO: Ahora solo llama a la nueva función central
...
// ... (El resto de las funciones: updateKPI, updatePriorityList, etc., no necesitan cambios)
...
            scales: { x: { beginAtZero: true, max: 100, title: { display: true, text: 'Probabilidad de Riesgo Alto (%)' } } },
            plugins: { legend: { display: false }, tooltip: { callbacks: { label: function(context) { return `Probabilidad: ${context.parsed.x.toFixed(1)}%`; } } } }
        }
    });
...
        .map(([uv, count]) => ({ uv, count, percentage: (count / periodData.length * 100).toFixed(1) }))
...
        return;
    }
    const scatterData = csvData.map(row => ({ x: row.Lluvia_Predicha_mm, y: row.Probabilidad_Riesgo_Alto * 100 }));
...
            responsive: true,
            maintainAspectRatio: false,
            scales: { x: { title: { display: true, text: 'Lluvia Predicha (mm)' }, beginAtZero: true }, y: { title: { display: true, text: 'Probabilidad de Riesgo Alto (%)' }, beginAtZero: true, max: 100 } },
            plugins: { tooltip: { callbacks: { label: function(context) { const data = csvData[context.dataIndex]; return [ `Unidad: ${data.Unidad_Vecinal}`, `Lluvia: ${data.Lluvia_Predicha_mm.toFixed(2)} mm`, `Prob. Riesgo: ${(data.Probabilidad_Riesgo_Alto * 100).toFixed(1)}%`, `Riesgo: ${data.Riesgo_Predicho}` ]; } } } }
        }
    });
